...
// Debugging the our Malloc is extremely noisy and usually undesired
...
inline Local<Object> wrap64(const Local<Context> &context, uint64_t val) {
    Isolate *isolate = context->GetIsolate();
    EscapableHandleScope scope(isolate);
    Local<ArrayBuffer> wrap = ArrayBuffer::New(isolate, 8);
    memcpy(wrap->GetContents().Data(), &val, 8);
    return scope.Escape(wrap);
}

inline uint64_t unwrap64(const Local<Context> &context, const Local<Value> &val) {
    Local<ArrayBuffer> wrap = val->ToObject(context).ToLocalChecked().As<ArrayBuffer>();
    internal_assert(!wrap->IsNullOrUndefined());
    internal_assert(wrap->IsArrayBuffer());
    internal_assert(wrap->ByteLength() == 8);
    uint64_t result;
    memcpy(&result, wrap->GetContents().Data(), 8);
    return result;
}
...
// TODO: although wasm will support int64 internall, AFAICT there is no way to pass them
// across the boundary, which
template<>
inline void LoadAndReturnScalar<void*>::operator()(const Local<Context> &context, const void *slot, ReturnValue<Value> val) {
    val.Set(wrap64(context, *(const uint64_t *)slot));
}
...
inline Local<Value> WrapScalar<void*>::operator()(const Local<Context> &context, const void *val_ptr) {
    uint64_t u = (uint64_t) *(void **)const_cast<void*>(val_ptr);
    return wrap64(context, u);
}

template<>
inline Local<Value> WrapScalar<uint64_t>::operator()(const Local<Context> &context, const void *val_ptr) {
    uint64_t u = *(const uint64_t *) val_ptr;
    return wrap64(context, u);
}
...
void add_extern_callbacks(const Local<Context> &context,
    ...
    for (const auto &it : jit_externs) {
        ...
        if (should_skip_extern_symbol(name)) {
            wdebug(0) << "SKIPPING "<<name<<"\n";
            continue;
        } else {
            wdebug(0) << "KEEPING "<<name<<"\n";
        }
        ...
        if (sig.is_void_return()) {
            ...
            *arg_types++ = ExternArgType{ {halide_type_int, 0, 0}, true, false };
            ...
            // TODO: the assertion below can be removed if we can find a way to marshal int64 values across the barrier
