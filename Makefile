...
# TODO: apparently no llvm_config flag to get canonical paths to tools
LLVM_STATIC_LIBS += -L $(LLVM_LIBDIR) $(shell $(LLVM_CONFIG) --link-static --libfiles lto option)
LLVM_STATIC_LIBS += $(LLVM_LIBDIR)/liblldCommon.a $(LLVM_LIBDIR)/liblldWasm.a
endif
...
$(BIN_DIR)/$(TARGET)/generator_aotwasm_%.js: $(ROOT_DIR)/test/generator/%_aottest.cpp $(FILTERS_DIR)/%.a $(FILTERS_DIR)/%.h $(RUNTIME_EXPORTED_INCLUDES) $(BIN_DIR)/$(TARGET)/runtime.a
	...
	EMCC_WASM_BACKEND=1 $(EMCC) $(GEN_AOT_CXX_FLAGS_WASM) -s WASM=1 -s SIMD=0 -s EXIT_RUNTIME=1 -s ENVIRONMENT=shell --source-map-base . $(filter %.cpp %.o %.a,$^) $(GEN_AOT_INCLUDES) $(GEN_AOT_LD_FLAGS_WASM) -o $@
